...
async def search(ctx, *, message):
    ...
    if cd != "BLACKLIST":
        ...
        print(f"Kirua of {message} by {ctx.author.name} | {ctx.author.id}.")
...
async def help(ctx):
    ...
    embed.set_author(name="SearchEngine Help 🆘")
    ...
    await ctx.send(embed=embed)
    print(f"SearchEngine Help Commands: by {ctx.author.name} | {ctx.author.id}.")
